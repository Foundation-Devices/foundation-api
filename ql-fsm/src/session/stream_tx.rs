...
impl StreamTx {
    ...
    pub fn poll_transmit(
        ...
        let budget_end = |start: u64| {
            start
                .saturating_add(max_payload as u64)
                .min(peer_max_offset)
        };

        // prefer the lowest lost bytes before sending new bytes
        if let Some(range) = self.retransmits.peek_min() {
            let mut end = range.end.min(budget_end(range.start));

            // extend only when lost bytes end where unsent bytes begin
            if end == range.end && range.end == self.unsent {
                end = self.end_offset().min(budget_end(range.start));
            }
            ...
            if end > range.start {
                ...
                // mark any new bytes in this frame as sent
                self.unsent = self.unsent.max(end);
        ...
        // send bytes that have not been sent yet
        if self.unsent < self.end_offset() {
            let end = self.end_offset().min(budget_end(self.unsent));
            if end > self.unsent {
        ...
        // send a fin after all data has been sent
        let final_offset =
            self.final_offset
                .as_mut()
                .filter(|TrackedFinalOffset { offset, state }| {
                    (*state == SendState::Lost || *state == SendState::Unsent)
                        && *offset <= peer_max_offset
                })?;
...
mod tests {
    ...
    #[test]
    fn lost_range_is_selected_before_unsent_bytes() {
        let mut tx = StreamTx::new();
    ...
    #[test]
    fn lost_range_coalesces_contiguous_unsent_bytes() {
        let mut tx = StreamTx::new();
        tx.append(Bytes::from_static(b"abc"));

        let first = tx.poll_transmit(3, u64::MAX).unwrap();
        tx.retransmit(first);
        tx.append(Bytes::from_static(b"def"));

        assert_eq!(
            tx.poll_transmit(6, u64::MAX),
            Some(StreamTxRange {
                offset: 0,
                len: 6,
                fin: false,
        ...
        assert_eq!(tx.poll_transmit(6, u64::MAX), None);
    }

    #[test]
    fn lost_range_coalesces_only_new_bytes_that_fit() {
        let mut tx = StreamTx::new();
        tx.append(Bytes::from_static(b"abc"));
    ...
    #[test]
    fn non_contiguous_lost_range_does_not_coalesce_unsent_bytes() {
        let mut tx = StreamTx::new();
        tx.append(Bytes::from_static(b"abcdef"));

        let first = tx.poll_transmit(3, u64::MAX).unwrap();
        let _second = tx.poll_transmit(3, u64::MAX).unwrap();
        tx.retransmit(first);
        tx.append(Bytes::from_static(b"ghi"));
