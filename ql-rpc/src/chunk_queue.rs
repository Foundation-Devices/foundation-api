...
impl ChunkQueue {
    ...
    pub fn try_take_tagged_part_header(&mut self) -> Result<Option<(u8, usize)>, Error> {
        let mut bytes = self.peek();
        let Ok(kind) = bytes.try_get_u8() else {
            return Ok(None);
        };
        let Some(len) = read_part_len_header(&mut bytes)? else {
            return Ok(None);
        };
    ...
    pub fn try_take_body(&mut self, len: usize) -> Option<DrainBuf<'_>> {
        if self.remaining < len {
            return None;
        }

        Some(DrainBuf::new(self, len))
    }
...
fn read_next_part_len<B: Buf>(bytes: &mut B) -> Result<Option<usize>, Error> {
    let Some(len) = read_part_len_header(bytes)? else {
        return Ok(None);
    };
...
fn read_part_len_header<B: Buf>(bytes: &mut B) -> Result<Option<usize>, Error> {
