...
use backup_shard::Shard as BackupShard;
use dcbor::CBOR;
use foundation_api::{
...
// Tests for backup-shard crate types
...
fn golden_backup_shard_v0() {
    ...
    let decoded_shard: BackupShard = decoded_cbor.try_into().unwrap();
...
fn golden_backup_shard_version_v0() {
    ...
    let decoded_cbor = CBOR::try_from_data(&bytes).unwrap();
    let decoded_version: ShardVersion = decoded_cbor.try_into().unwrap();
